...
public class CapCheckExecutor implements Runnable {
  ...
  // The maximum number of devices for a bulk user check
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      // Check that the minimum number of device threshold is reached.
      if (totalInputUsers < MIN_DEVICES) {
        logger.severe("Your input file has only " + totalInputUsers + " unique devices. "
            + "The bulk capability check API will not "
            + "return results for a list of less than " + MIN_DEVICES + ".");
  ...
  /**
   * Performs a batch user capability check. If the list of phone numbers has more than 10,000 values, the list is
   * split into sublists of a maximum 10,000 numbers each. One batch call is made for each sublist.
   *
